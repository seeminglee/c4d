...
#        DESC_VERSION_XL=2
    ...
    def __init__(self):
        self.lib = None
        self.revdicts = None  # list of dictionaries to get key from value
        self.init_lib()
    ...
    def init_lib(self):
        ...
        """
        self.lib = dict()
        # constrct five dicts from the start
        self.revdicts = list()
        for i in xrange(5):
            self.revdicts.append(dict())
        for k, v in self._rawlib.items():
            ktuple = tuple(k.split('_'))
            self.lib[ktuple] = v
            # construct reverse lookup dictionary
            level = len(ktuple)
            self.revdicts[level][v] = k

    def decode(self, id):
        for revdict in self.revdicts:
            if id in revdict.keys():
                return revdict[id]
        return None
